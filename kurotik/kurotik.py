...
class KuroTik:
    def __init__(self):
        self.__PARAM_SCHEMA_CONNECTION = {
            "host": str,
            "username": str,
            "password": str,
            "port": int,
    ...
    def __setConnection(self, **kwargs):
        self.__validate_params(self.__PARAM_SCHEMA_CONNECTION, kwargs)
    ...
    def __executeCommands(self, **kwargs):
        self.__validate_params(self.__PARAM_SCHEMA_EXECUTE, kwargs)
        if kwargs.get('x', None):
            return self.api.get_resource(kwargs['x'])
        return self.api.get_resource(kwargs.get(
            'resources', '/'
        )).call(kwargs.get(
            'function', 'ping'
        ), kwargs.get(
            'params', {'address': '8.8.8.8', 'count': '4'}
        ))
